...
# Default score-blend weights (recency, importance, relevance) per query mode.
_DEFAULT_WEIGHTS = {
...
def _resolve_weights(config, mode):
    """Resolve (w_r, w_i, w_v) for a query mode, honouring config overrides.

    Falls back to the built-in defaults when no config or no override is set.
    Shared by the JSONL and Neo4j stores so both honour tuned weights.
    """
    default = _DEFAULT_WEIGHTS.get(mode, _DEFAULT_WEIGHTS['semantic'])
    if config is None:
        return default
    attr = 'structured_weights' if mode == 'structured' else 'semantic_weights'
    weights = getattr(config, attr, None)
...
class MemoryStore:
    ...
    def __init__(self, jsonl_path, config=None):
        """Initialize store with path to JSONL file.

        The file is not created until the first write operation.

        Args:
            config: Optional MemoryConfig — supplies tunable scoring weights.
        """
    ...
    def _upsert_inner(self, memory_dict):
        ...
        # Reasoning: REPLACE (matches the Neo4j store + schema Rule 6/9). The .md file is the
        # accumulator — the working-memory rule appends in the file and the store mirrors the full
        # text; appending here doubled chain reasoning when the hook upserts the already-full .md text.
...
def get_store(jsonl_path=None, config=None, require_neo4j=None):
    ...
    return MemoryStore(jsonl_path, config=config)
