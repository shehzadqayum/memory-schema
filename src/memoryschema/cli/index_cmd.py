...
@click.option("--all-spaces", "all_spaces", is_flag=True,
              help="Backfill ALL 7 spaces + divergence profile into the active store (Neo4j+JSONL).")
@click.option("--json", "as_json", is_flag=True, help="Output as JSON.")
@click.pass_obj
def embed(config, prefix, embed_all, coverage, batch_size, dry_run, space, all_spaces, as_json):
    ...
    Example:
        ...
        memoryschema embed --all-spaces          # backfill multi-space + divergence
    ...
    if coverage:
        ...
        multispace = sum(1 for e in entries
                         if e.get('embeddings') and any(s != 'default' for s in e['embeddings']))
        pct = (embedded / total * 100) if total > 0 else 0
        ms_pct = (multispace / total * 100) if total > 0 else 0
        if as_json:
            click.echo(json.dumps({"total": total, "embedded": embedded, "coverage_pct": round(pct, 1),
                                   "multispace": multispace, "multispace_pct": round(ms_pct, 1)}))
        else:
            click.echo(f"Total:       {total:,}")
            click.echo(f"Embedded:    {embedded:,}")
            click.echo(f"Coverage:    {pct:.1f}%")
            click.echo(f"Multi-space: {multispace:,} ({ms_pct:.1f}%)")
        return

    if all_spaces:
        from memoryschema.reembed import reembed_all_spaces
        result = reembed_all_spaces(prefix=prefix or "", config=config, dry_run=dry_run)
        if as_json:
            click.echo(json.dumps(result))
        else:
            click.echo(f"Matched:      {result['matched']:,}")
            click.echo(f"Embedded:     {result['embedded']:,} (all spaces + divergence)")
            if result.get('skipped_empty'):
                click.echo(f"Skipped:      {result['skipped_empty']:,} (no default text)")
