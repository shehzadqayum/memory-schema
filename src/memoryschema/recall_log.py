...
def log_recall(config, query, results, backend, degraded=False, now=None):
    ...
    try:
        ...
        # Log ALL served hits (bounded by the recall --limit), not a fixed top-N. The log is a record
        # of what was SERVED: an entity absent here was never served, not merely rank-capped — which
        # is exactly the distinction the dream report's `never_surfaced` needs (a consistently rank-11+
        # entity is no longer falsely flagged as dead weight). Probe rows are logged like any other.
