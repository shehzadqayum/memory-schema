...
- If `format_v5.is_v5_content(content)` — the stripped content starts with a `---`
  fence — the file parses as v5 and **never** falls through to XML. `is_v5_content`
  checks `content.lstrip()` for a leading `---\n` / `---\r\n` / bare `---`. (Known
  edge: `str.lstrip()` does not strip a U+FEFF BOM, so a BOM-prefixed v5 file fails
  dispatch; `parse_v5_content` itself strips BOM if called directly.)
- Otherwise the v4 XML path runs (§3.4).
- Write-side selection: `write_index.create_entity_file` emits **v5 iff env
  `MEMORYSCHEMA_V5 == "1"`** (exact string), else v4. Exception: the chain-file
  bootstrap (first `chain step` on a missing chain file) always writes a v5 skeleton.

### 3.2 v5 format (current)

File shape: `---` fence · YAML-subset frontmatter · `---` fence · markdown body.

**Frontmatter grammar** (deliberate zero-dependency YAML subset, parsed line-by-line):

- Blank lines and `#`-comment lines skipped.
- `relations:` (exact stripped match) enters relations mode: each following line matching
  `^-\s+([A-Z_]+)\s+([A-Za-z0-9][A-Za-z0-9-]*)\s*$` appends `{"type", "target"}`; the
  first non-matching line exits the mode and is parsed as a scalar.
- Scalar lines: `key: value` (not starting with space or `-`); value stripped of
  surrounding `"` then `'`. Unknown keys are ignored (not carried to the entity).
- The closing fence is the first line whose strip equals `---`. **Unterminated fence →
  the whole parse returns `None`.**

**Discriminator**: `schema: 5` is REQUIRED. Any other value (or absence) → parse returns
`None`, so ordinary frontmatter markdown (wiki notes) is skipped exactly like
non-entity files.
...
| `key` | fact key for temporal validity (e.g. `EURUSD.bias`) (§4.6) |
| `valid_from` | ISO date — validity interval start |
| `superseded_at` | ISO date — validity interval end (set by supersession) |
| `superseded_by` | successor entity name |
| `promoted_to` | standing-surface marker (e.g. `CLAUDE.md#section`) (§8.3) |
| `relations` | list of `- TYPE target` lines |

**Body grammar**: split on `^## <Title>` headings (exactly two `#`; `###` is content;
duplicate headings merge). Content before the first heading is the lead; the
**description is the first blank-line-separated paragraph of the lead** (newlines
flattened to spaces; later lead paragraphs are discarded).

| section | parse |
|---------|-------|
| `## Summary` | prose — the evolving chain summary (replaced by `--desc`) |
| `## Observations` | bullets: `- ` opens an item; following non-bullet non-empty lines join with a space; blanks are no-ops |
| `## Log` | bullets — ordered `Step N: …` entries (chains) |
| `## Reasoning` | prose — narrative; appended-to with `\n\n---\n` separators |
| `## Prompt` | prose — the triggering input |
| `## Chain` | prose — chain context |
| `## Notes` | prose → the `body` field (v4 body-after-entity parity) |

Unknown `##` sections parse but are **discarded on roundtrip** — do not invent sections.

**Parsed dict**: `observations` is the flattened index view — atomic observations plus
the log entries (the file keeps them distinct; both stores index the flattened list).
`None`-valued keys are dropped. Differences vs a v4 parse: no `filepath`/`related`/
`confidence` keys; `status` always present; `importance` absent (not `None`) when unset.
**`confidence` is not representable in v5** — it is v4-only, retired.

**Serializer** (`serialize_v5`) emits, in order: the fence; `schema: 5`; scalars in fixed
order `type, importance, status, project, key, valid_from, superseded_at, superseded_by,
promoted_to` (suppressing the defaults `type: semantic` and `status: active`);
`relations:` block; fence; description; then sections `Summary, Observations, Log,
Reasoning, Prompt, Chain, Notes` (empty ones omitted; observation bullets that duplicate
log entries are deduped out of Observations). **Nothing is escaped** — a literal
`</memory:entity>` in an observation roundtrips verbatim.

**Roundtrip guarantees** (pinned by `tests/test_format_v5.py`): parse→serialize→parse
identity for description, summary, log, prompt, chain, relations, importance,
observations; reasoning is whitespace-normalized. Lossy edges: extra lead paragraphs,
unknown sections, multi-line bullets (reflowed to one line), newlines in the description.

**Example (complete v5 entity)**
...
- atomic fact one — raw < > & are safe here
- atomic fact two

## Reasoning

Why this is believed; alternatives considered.
```

### 3.3 v5 validation model

There is **no rule-based validator for v5**. Well-formedness is parse-based:
`parse_v5_content(...) is None` is the single criterion, and every deterministic writer
re-parses its own output before committing (create refuses to write; v5 append validates
before writing; v4 append validates after and rolls back). Malformed v5 files surface in
`sync`/`reconcile` as unparseable (reconcile refuses to run against them). Note the
consequences for legacy commands: `memoryschema validate` and the `write` command run the
v4 validator, which reports any v5 file as `V1` (no XML entity found) — `validate`/`write`
are v4-only; the hook quietly skips malformed v5 files (§9.4).

### 3.4 v4 XML format (legacy — parses, not authored)

- Entity block: first `(<memory:entity\b.*?</memory:entity>)` DOTALL match anywhere in
  the file; text after the closing tag is `body`.
- Parsed by stripping the `memory:` namespace prefix literally and feeding stdlib
  `ElementTree`. Parse error or missing `name` attribute → `None`.
- Attributes: `name` (required), `schema` (int, default 1), `type` (default `semantic`),
  `status` (carried **only when declared** — a re-index never overrides store lifecycle),
  `importance` (int or None), `confidence` (int or None; v4-only).
- Children: `description`, `observations`/`observation`, `prompt`, `reasoning`, `chain`,
  `relations`/`relation target= type=`, `project` (else derived from a
  `projects/<kebab>/` path segment, nested → dot-joined).
- Escaping (write side): `xml.sax.saxutils.escape` — `&`, `<`, `>` only (not quotes).
- v4 rule validator (`validator.py`): V1–V12 structure (V8 retired), R1–R6 relations
  (R7 cycle detection lives in the store), F3 filesystem, Q1–Q8 strict-mode quality
  checks. Applies to v4 files only.

### 3.5 Relations

Seven active types: `USES`, `MODIFIES`, `SUPERSEDES`, `DEPENDS_ON`, `INFORMS`,
`CONTRADICTS`, `MITIGATES` (+ deprecated `PARENT_OF`/`CHILD_OF` — use the `project`
field). No self-references; deduped by (target, type). Side effects at the store layer
(§5.1): `SUPERSEDES` flips the target's status to `superseded` (with cycle detection);
`CONTRADICTS` auto-creates the symmetric edge; `MITIGATES` dampens the target's score
(×0.95) and is audit-logged.

Three connection types at retrieval time: **relations** (authored, forward),
**backlinks** (computed reverse), **associations** (computed embedding k-NN, k=10).

### 3.6 Entity types & status

`type` is free-form; scoring recognizes `semantic` (recency floor 0.6), `episodic`
(standard decay), `procedural` (access-reinforced recency) — §6.3. `status` ∈
`active | superseded | archived | quarantined`; non-active entities are excluded from
recall/search/L0 by default (`--include-inactive` reveals them) and superseded entries
