...
class TestUpsert:
    ...
    def test_chain_reasoning_replaces(self, store):
        """Chain entities REPLACE reasoning on upsert (matches the Neo4j store + schema Rule 6/9).
        The .md file is the accumulator; the store mirrors the full text. Appending here previously
        DOUBLED chain reasoning, because the hook upserts the already-full .md text each time."""
        store.upsert({'name': 'chain-test', 'schema': 2, 'description': 'Chain',
                       'reasoning': 'Step 1'})
        # re-upsert the FULL accumulated text (as the hook does from the .md) — must not double
        store.upsert({'name': 'chain-test', 'reasoning': 'Step 1\n---\nStep 2'})
        entry = store.get('chain-test')
        assert entry['reasoning'] == 'Step 1\n---\nStep 2'
        assert entry['reasoning'].count('Step 1') == 1   # not doubled
        assert entry['reasoning'].count('---') == 1
